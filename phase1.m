...
l_walk = 30;           % Length of walking path (m)
...
% rotation_center = -1.5*l_thigh;     % center of leg endpoint rotation, measured from body-leg hinge point.
rotation_center = -0.95*l_thigh;

motion_path = [0 l_thigh/2 0; l_thigh/2 0 0; 0 -l_thigh/2 0; -l_thigh/2 0 0];
% circle, radius = l_thigh/2.
...
omega = -0.4*pi; % Angular velocity (rad/s)
tf = 10;        % Final time (s)
...
xlabel('Time (s)');
ylabel('\dot{\theta_2} (degrees/s)');
title('Angular Velocity {\omega_2} over Time');
grid on;

theta_l_up = timeseries(theta1',t);
theta_l_down = timeseries(theta2',t);


%% Getting the angular displacement for front leg
...
theta_r_up = timeseries(theta1',t);
theta_r_down = timeseries(theta2',t);
